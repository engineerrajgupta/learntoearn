    ...
    <script>
        ...
        if (chatMessagesContainer && chatMessages && messageInput && sendButton) {
            ...
            function sendMessage() {
                 ...
                 if (message) {
                     addMessage(message, true); // Display user message (using textContent)
                     messageInput.value = ''; // Clear input
                     messageInput.style.height = 'auto'; // Reset height before calculating new height
                     messageInput.style.height = `${messageInput.scrollHeight}px`; // Reset height for next input
                     sendButton.disabled = true; // Disable button after sending
                     messageInput.disabled = true; // Disable input while waiting

                     // Add a subtle "Bot is thinking..." indicator (optional)
                     ...
                     // --- BACKEND CALL ---
                     fetch('chatbot_handler.php', { // Make sure path is correct
                         method: 'POST',
                         headers: {
                     ...
                     .then(response => {
                         // Remove thinking indicator immediately once response starts coming back
                     ...
                     .then(data => {
                         if (data && data.response) {
                             // Add bot response, allowing HTML formatting
                     ...
                     .catch(error => {
                         document.getElementById('thinking-indicator')?.remove(); // Ensure removal on error too
                     ...
                     .finally(() => {
                         ...
                         // Keep send button disabled until input changes
                         messageInput.focus();
                         // Auto-adjust height again in case something went wrong
            ...
            messageInput.addEventListener('keydown', (event) => {
             ...
             window.addEventListener('load', () => {
                 setTimeout(() => {
                     addMessage("Hello! 👋 I'm your LearnToEarn AI Assistant. How can I help you today?", false, true);
